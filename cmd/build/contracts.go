...
func convertToV3(ctx context.Context, r io.Reader) (map[string]any, error) {
	...
	schema, err := io.ReadAll(r)
	if err != nil {
		return nil, err
	}
	if err := json.Unmarshal(schema, &v2); err != nil {
		return nil, err
	}
		...
		if t, ok := m["type"]; ok {
			if ts, ok := t.([]any); ok {
				if slices.Contains(ts, "null") {
					m["type"] = slices.DeleteFunc(ts, func(t any) bool {
						return t == "null"
					})[0]
					m["nullable"] = true
				}
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, SwaggerConverterURL, bytes.NewReader(body))
	if err != nil {
		return nil, err
	}
	...
	walk("", v3, func(k string, m map[string]any) {
		switch k {
		case "Vector":
