...
func TestRESTResponses(t *testing.T) {
	for _, tt := range testkit.WithOnly(t, []struct {
		...
		{
			// createdAt, lastUsedAt, and apiKeyFirstLetters are missing
			// from rest.DBUserInfo, hence the raw JSON.
			name: "get user info with last used time",
			body: &map[string]any{
				"userId":             "john-malkovich",
				"dbUserType":         rest.DBUserInfoDbUserTypeDbUser,
				"active":             true,
				...
				"createdAt":          testkit.Now.UTC(),
				"lastUsedAt":         testkit.Now.UTC(),
			},
			dest: new(api.UserInfo),
			want: &api.UserInfo{
				ID:                 "john-malkovich",
				...
				Roles:              []string{"external"},
				APIKeyFirstLetters: "abr",
				CreatedAt:          testkit.Ptr(testkit.Now.UTC()),
				LastUsedAt:         testkit.Ptr(testkit.Now.UTC()),
			},
		},
		{
			name: "get user info with null last used time",
			body: &map[string]any{
				"userId":             "john-malkovich",
				"dbUserType":         rest.DBUserInfoDbUserTypeDbUser,
				"active":             true,
				"roles":              []string{"external"},
				"apiKeyFirstLetters": nil,
				"createdAt":          nil,
				"lastUsedAt":         nil,
			},
			dest: new(api.UserInfo),
		...
		{
			name: "list db users with last used time",
			body: []map[string]any{{
				"userId":             "john-malkovich",
				"dbUserType":         rest.DBUserInfoDbUserTypeDbUser,
				"active":             true,
