...
func TestVectorObjectSearch(t *testing.T) {
	...
	if !ok {
		fmt.Println("Failed to retrieve build info")
		return
	}
	...
	t.Run("batch import", func(t *testing.T) {
		tests := []struct {
			...
			vector_index_config map[string]interface{}
		}{
			{
				name:              "dataset with hnsw index",
			...
			// {
			// 	name:              "dataset with bq compression",
			// 	vector_index_type: "hnsw",
			// 	vector_index_config: map[string]interface{}{
			// 		"multivector": map[string]interface{}{
			// 			"enabled": true,
		...
		hdf5_file := "/Users/rodrigo/workspace/datasets/custom_Multivector_lotte-recreation-reduced_1000_-1.hdf5"
		//hdf5_file := "/Users/rodrigo/Downloads/custom_Multivector_lotte-recreation-reduced-vl.hdf5"
			...
			t.Run(tt.name, func(t *testing.T) {
				...
				vectorConfig := map[string]models.VectorConfig{
					...
					"colbert": {
						...
						VectorIndexType:   tt.vector_index_type,
						VectorIndexConfig: tt.vector_index_config,
					},
				}
				class := &models.Class{
					Class: className,
				...
				vectors := loadVectors(hdf5_file)
				//queries := loadHdf5Queries(hdf5_file, "queries")
				...
				t.Run("vector search", func(t *testing.T) {
					...
					for _, obj := range objects {
						...
						// 	WithVector(query_vector["colbert"]).WithTargetVectors("colbert")
